...
const AdminEditProductModal = ({ open, onClose, product }) => {
  ...
  useEffect(() => {
    if (product) {
      setFormData({
        ...
        embeddedVideoLink: product.embeddedVideoLink || "", // prefill
        description: {
  ...
  const handleUpdate = async () => {
    const { name, price, markedPrice, rating, categoryName, tags, description, inStock, isBestSelling, embeddedVideoLink } = formData;
    ...
    if (embeddedVideoLink && !isValidURL(embeddedVideoLink)) {
      return toast.error("Please enter a valid URL for the embedded video link.");
    }
    ...
    updatedFormData.append("isBestSelling", isBestSelling);
    if (embeddedVideoLink) updatedFormData.append("embeddedVideoLink", embeddedVideoLink);
  ...
  return (
    <Modal open={open} onClose={onClose}>
      ...
      >
        ...
        {/* Embedded Video Link */}
        <TextField fullWidth label="Embedded Video Link (optional)" name="embeddedVideoLink" value={formData.embeddedVideoLink} onChange={handleChange} margin="normal" />
